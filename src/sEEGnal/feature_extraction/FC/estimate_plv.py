...
Estimate phase-locking value in sensor or source space.

Functions
---------
estimate_plv
    Compute and save PLV for the configured sensor and source spaces.
...
def estimate_plv(config, BIDS):
    ...
    -------
    metadata : dict
        Metadata describing the last estimated PLV output.
    """
    ...
    epochs = prepare_eeg(config, BIDS, raw=raw, freq_limits=freq_limits_signal, metadata_badchannels=True, exclude_badchannels=True, interpolate_badchannels=False, epoch_definition=config['feature_extraction']['plv']['epoch_definition'])
    ...
    if 'sensor' in config['feature_extraction']['plv']:
        ...
        for iband,current_band in enumerate(params['freq_bands_name']):
            ...
            # plv
            good_band_plv_vector = compute_plv(
            ...
            metadata = {
                ...
                "description": "Upper triangular PLV matrix values obtained with numpy.triu_indices(n_nodes, k=1); connections involving bad channels are NaN"
            }
    ...
    if 'source' in config['feature_extraction']['plv']:
        ...
        for iband, current_band in enumerate(params['freq_bands_name']):
            ...
            require_finite_source_values(
                band_plv_vector,
                f'Source PLV for band {current_band}',
            )
            ...
            metadata = {
                ...
                "band_name": current_band,
                "vectorization": "numpy.triu_indices(n_nodes, k=1)",
                "description": "Upper triangular source PLV matrix values obtained with numpy.triu_indices(n_nodes, k=1)",
                **source_metadata,
            }
